...
public:
    ...
    static queuef* create(Funcs... funcs) {
        queuef* qf = new queuef;
        (qf->functions.push(funcs), ...);
        return qf;
    }
...
int main() {
    auto doubleQueue = queuef<double>::create(
    ...
    auto stringQueue = queuef<std::string>::create(
